...
class ups extends base {
  ...
  function __construct() {
    ...
    if (MODULE_SHIPPING_UPS_STATUS == 'True') {
      if (IS_ADMIN_FLAG) {
        $chk_keys = $this->keys();
        $chk_sql = $db->Execute("select * from " . TABLE_CONFIGURATION . " where configuration_key like 'MODULE\_SHIPPING\_UPS\_%' ");
        if ((MODULE_SHIPPING_UPS_VERSION != '2015-12-30') || (sizeof($chk_keys) != $chk_sql->RecordCount())) {
          $this->title .= '<span class="alert">' . ' - Missing Configuration Settings or Out of date; You should reinstall!' . '</span>';
          $this->enabled = FALSE;
        }
        $new_version_details = plugin_version_check_for_updates(1293, '2015-12-30');
    ...
    if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_UPS_ZONE > 0) ) {
      $check_flag = false;
      $check = $db->Execute("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_SHIPPING_UPS_ZONE . "' and zone_country_id = '" . (int)$order->delivery['country']['id'] . "' order by zone_id");
      while (!$check->EOF) {
  ...
  function check() {
    ...
    if (defined('MODULE_SHIPPING_UPS_STATUS')) {
      if (!defined('MODULE_SHIPPING_UPS_VERSION')) {
        $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('UPS Version Date', 'MODULE_SHIPPING_UPS_VERSION', '2015-12-30', 'You have installed:', '6', '0', 'zen_cfg_select_option(array(\'2015-12-30\'), ', now())");
      }
      if (!defined('MODULE_SHIPPING_UPS_HANDLING_METHOD')) {
    ...
    return (defined('MODULE_SHIPPING_UPS_STATUS'));
  }
  /**
   * Install this module
   *
