...
class ot_coupon {
  ...
  function no_sale_or_special($product_id) {
       
    if (DISABLE_COUPON_SALE_CHECK == 'true') return true; 

    if (zen_get_products_special_price($product_id, false)!= false) return false; 
    if (zen_get_products_special_price($product_id, true)!= false) return false; 
    return true; 
  }
...
//          zen_redirect(zen_href_link(FILENAME_CHECKOUT_PAYMENT, '', 'SSL',true, false));
        ...
        if ($foundvalid == true) {
          ...
          for ($i=0; $i<sizeof($products); $i++) {
            if (is_product_valid($products[$i]['id'], $coupon_result->fields['coupon_id']) && $this->no_sale_or_special($products[$i]['id'])) {
              $foundvalid = true;
              continue;
            }
          }
...
//        if (strval($coupon_total) >= $coupon->fields['coupon_minimum_order']) {
        if (strval($coupon_total_minimum) >= $coupon->fields['coupon_minimum_order']) {
          ...
          if ($coupon->fields['coupon_product_count'] && ($coupon->fields['coupon_type'] == 'F' || $coupon->fields['coupon_type'] == 'O')) {
            ...
            for ($i=0, $n=sizeof($products); $i<$n; $i++) {
              if (is_product_valid($products[$i]['id'], $coupon->fields['coupon_id']) && $this->no_sale_or_special($products[$i]['id'])) {
...
//    print_r($od_amount);
  ...
  function get_order_total($couponCode)
  {
    ...
    $products = $_SESSION['cart']->get_products();
    for ($i=0, $n=sizeof($products); $i<$n; $i++) {
      $is_product_valid = (is_product_valid($products[$i]['id'], $couponCode) && is_coupon_valid_for_sales($products[$i]['id'], $couponCode) && $this->no_sale_or_special($products[$i]['id']));
      $GLOBALS['zco_notifier']->notify(
