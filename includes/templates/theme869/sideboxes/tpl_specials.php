 ...
 */
  ...
  while (!$random_specials_sidebox_product->EOF) {
    ...
    $content .= '<a class="name" href="' . zen_href_link(zen_get_info_page($random_specials_sidebox_product->fields["products_id"]), 'cPath=' . zen_get_generated_category_path_rev($random_specials_sidebox_product->fields["master_categories_id"]) . '&products_id=' . $random_specials_sidebox_product->fields["products_id"]) . '">' . substr(strip_tags($random_specials_sidebox_product->fields['products_name']), 0, 36) . '</a>';
	$content .= '<div class="text">';
	$content .= substr(strip_tags($random_specials_sidebox_product->fields['products_description']), 0, 40) ;
	$content .= '</div>';
	$content .= '</div>';
  $content .= '</div>';
