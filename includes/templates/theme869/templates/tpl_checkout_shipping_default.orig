...
<table class="shipping_wrapper resume table table-bordered">
<?php
...
<?php echo '<tr><td class="shipping_radio_button">' . zen_draw_radio_field('shipping', $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id'], $checked, 'id="ship-'.$quotes[$i]['id'] . '-' . $quotes[$i]['methods'][$j]['id'].'"') . '</td>'; ?>

<?php echo '<td class="shipping_img"><div class="shipping_img" id="'.$quotes[$i]['id'].'_img"></div></td>'; ?>

<td><label class="shipping_label" for="ship-<?php echo $quotes[$i]['id'] . '-' . $quotes[$i]['methods'][$j]['id']; ?>" class="checkboxLabel" ><?php echo $quotes[$i]['methods'][$j]['title']; ?></label></td>

<?php
            if ( ($n > 1) || ($n2 > 1) ) {
?>
<td><div class="important forward price"><?php echo $currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], (isset($quotes[$i]['tax']) ? $quotes[$i]['tax'] : 0))); ?></div></td>
<?php
            } else {
?>
<td><div class="important forward price"><?php echo $currencies->format(zen_add_tax($quotes[$i]['methods'][$j]['cost'], $quotes[$i]['tax'])) . zen_draw_hidden_field('shipping', $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id']); ?></div></td>
<?php
            }
?>
    </tr>
...
<div class="buttonRow forward"><?php echo zen_image_submit('', BUTTON_CONTINUE_ALT); ?></div>
