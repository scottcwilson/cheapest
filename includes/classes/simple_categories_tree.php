...
class SimpleCategoriesTree{
	...
	function init(){
		if(SCT_REBUILD_TREE != 'false' || count($this->category_tree) == 0){
			...
			$categories_query = "select c.categories_id, c.categories_image, cd.categories_name, cd.categories_description, c.parent_id
	...
	function buildCategoryString($parent_html = 'div', $child_html = 'span', $categories_id = 0, $default_current_id = 0,
	...
	function __buildCategoryString($categories_id, $level, $max_level, $strict){
		...
		if(($level < $max_level) || $is_allowed_to_go_deeper){
			...
			if($level > 0){
				...
				$result = str_replace(array("{class$level}","{name$level}","{link$level}", "{id}"), array($class, $this->category_tree[$categories_id]['name'], $this->_buildLink($categories_id), "sct_$categories_id"), $child_html);
			}
			else 
				$result = "{child$level}";
	...
	function _buildClass($categories_id, $level){
		...
		//global $cPath; //origin - why use global for $cPath ?
		$cPath = '';
		if(isset($_GET['cPath']))
			$cPath = $_GET['cPath'];
		...
		$active_class = ' no_active';
		foreach($current_categories_array as $key => $category_id) {
			if($categories_id == $category_id) {
				$active_class = ' active'.++$key;
			}
		}
		$class .= $active_class;
	...
	/**
   * Get all children categories from parent category
   * @param categoryId int
   * @param categoryTree object
   * @return childCategoriesArray array
   */
  function getChildrenCategories($categories_id) {
    if (zen_not_null($this->category_tree[$categories_id]['sub_cats'])){
  		foreach($this->category_tree[$categories_id]['sub_cats'] as $key => $child) {
  			$children_categories[] = $this->getChildrenCategories($child, $this->category_tree);
  		}
  		return implode(',', $children_categories);
  	} else {
  		return $categories_id;
  	}
  }
