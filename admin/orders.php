...
if (zen_not_null($action) && $order_exists == true) {
  switch ($action) {
      // bof Order Reminder 
    case 'reset_update_reminder': 
        $oID = zen_db_prepare_input($_GET['oID']);
        $update_reminder_query = "UPDATE " . TABLE_ORDERS . " SET reminder_email='', reminder_delay=0, reminder_sent=0, reminder_sent_date='0001-01-01' WHERE orders_id='" . $_GET['oID'] . "'"; 
    ...
    case 'update_reminder': 
        $oID = zen_db_prepare_input($_GET['oID']);
        $reminder_email = zen_db_prepare_input($_POST['reminder_email']);
        $reminder_delay = (int)zen_db_prepare_input($_POST['reminder_delay']);
        $update_reminder_query = "UPDATE " . TABLE_ORDERS . " SET reminder_email='" . addslashes($reminder_email) . "', reminder_delay='" . $reminder_delay . "' WHERE orders_id='" . $_GET['oID'] . "'"; 
        $db->Execute($update_reminder_query); 
        zen_redirect(zen_href_link(FILENAME_ORDERS, zen_get_all_get_params(array('action')) . 'action=edit', 'NONSSL'));
      break; 
      // eof Order Reminder 
