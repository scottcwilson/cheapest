<?php

define('FILENAME_PAGE_5', 'page_5');
define('FILENAME_PAGE_6', 'page_6');
define('FILENAME_PAGE_7', 'page_7');
define('FILENAME_USPS_TRACKING', 'usps_tracking');

define('FILENAME_DEFINE_PAGE_5', 'define_page_5');
define('FILENAME_DEFINE_PAGE_6', 'define_page_6');
define('FILENAME_DEFINE_PAGE_7', 'define_page_7');
define('FILENAME_DEFINE_USPS_TRACKING', 'define_usps_tracking');